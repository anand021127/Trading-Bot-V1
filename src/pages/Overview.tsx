...
import {
  ...
  RefreshCw, Play, Square, Zap, Wifi, WifiOff, Heart, Moon, KeyRound,
} from 'lucide-react'
...
export default function Overview() {
  ...
  const load = async () => {
    try {
      ...
      if (isAuthError(e)) {
        setError('Upstox access token expired or invalid (HTTP 401).')
      } else if (consecutiveErrors.current >= ERROR_THRESHOLD) {
        setError(e instanceof Error ? e.message : 'Backend connection unavailable')
      }
    } finally {
      setLoading(false)
    }
  ...
  // Scanner status: use heartbeat data when available, fall back to boolean
  const scannerStatus = scannerHealth?.scanner_status ?? (data?.scanner?.is_running ? 'RUNNING' : 'STOPPED')
  const scannerOk = scannerStatus === 'RUNNING'
  const scannerLabel = (() => {
    if (scannerStatus === 'RUNNING') {
      const ago = scannerHealth?.last_scan_seconds_ago
      if (ago != null) return `Running (${Math.round(ago)}s ago)`
      return 'Running'
    }
    if (scannerStatus === 'DEGRADED') return 'Degraded'
    if (scannerStatus === 'STARTING') return 'Starting…'
    return data?.scanner?.is_running ? 'Running' : 'Stopped'
  })()

  // Market data status
  const marketDataStatus = sys?.market_data_status ?? (sys?.websocket_connected ? 'LIVE' : 'UNAVAILABLE')
  const marketDataOk = marketDataStatus === 'LIVE'

  // Bot health overall
  const botHealthStatus = health?.bot_status ?? 'UNKNOWN'
  const botUptime = health?.uptime_seconds ?? botStatus?.uptime_seconds ?? 0
  ...
  if (error && !data) return (
    <div className="bg-red-950/30 border border-red-800/50 rounded-xl p-6 text-red-300 text-sm space-y-3">
      <div className="flex items-center gap-2 font-medium text-base text-red-200">
        <AlertTriangle size={18} className="text-red-400" />
        {isAuthExpired ? 'Upstox Authentication Expired' : 'Backend Connection Unavailable'}
      </div>
      <div className="text-red-400/80 text-xs">{error}</div>
      <div className="text-[11px] text-red-400/60 leading-relaxed">
        {isAuthExpired ? (
          <span>Your Upstox access token has expired. Please go to Settings and enter a refreshed access token.</span>
        ) : (
          <span>This does NOT mean the scanner or trading engine stopped — the backend process may still be running. The dashboard cannot reach it right now.</span>
        )}
      </div>
      <div className="flex items-center gap-3 pt-2">
        <button
          ...
          className="px-3 py-1.5 bg-red-800/60 hover:bg-red-700 text-white text-xs font-medium rounded-lg transition-colors"
        >
          Retry Connection
        </button>
        {isAuthExpired && (
          <button
            ...
            className="px-3 py-1.5 bg-amber-600 hover:bg-amber-500 text-slate-950 text-xs font-medium rounded-lg transition-colors flex items-center gap-1.5"
          >
            <KeyRound size={12} />
            Go to Settings
          </button>
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              <div className="text-amber-300/70 text-[11px] mt-0.5">
                Upstox access tokens expire daily at 3:30 AM IST. Re-authenticate to access real-time market ticks and trading.
              </div>
            </div>
          </div>
          <button
      ...
      {/* Transient error banner — shown when we have stale data but API is failing */}
      {error && data && !isAuthExpired && (
        <div className="flex items-start gap-2.5 bg-amber-950/30 border border-amber-700/50 rounded-xl px-4 py-2.5">
          <AlertTriangle size={14} className="text-amber-400 flex-shrink-0 mt-0.5" />
          <div>
            <div className="text-xs font-medium text-amber-300">Backend temporarily unreachable</div>
            <div className="text-[10px] text-amber-400/60 mt-0.5">
              Showing last known data. The bot and scanner may still be running — this is a dashboard connection issue, not a bot failure.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            ...
            {botUptime > 0 && (
              <> · Uptime: <span className="text-slate-400">{Math.floor(botUptime / 3600)}h {Math.floor((botUptime % 3600) / 60)}m</span></>
            )}
          </p>
        </div>
      ...
      {/* System health — real health states, not just booleans */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          {
          ...
          {
            label: 'API Health',
            ok: sys?.api_health === 'ok',
            val: sys?.api_health ?? 'unknown',
            color: sys?.api_health === 'ok' ? 'text-emerald-400' : 'text-amber-400',
            dotColor: sys?.api_health === 'ok' ? 'bg-emerald-400' : 'bg-amber-400',
          },
          {
            label: 'Configured Universe',
            ok: (data?.universe?.watching_count ?? 0) > 0 && scannerOk,
            val: `${data?.universe?.watching_count ?? 0} underlyings (${data?.universe?.mode ?? 'OPTIONS'})${scannerOk ? '' : ' — scanner ' + scannerStatus.toLowerCase()}`,
            color: (data?.universe?.watching_count ?? 0) > 0 && scannerOk ? 'text-emerald-400' : 'text-amber-400',
            dotColor: (data?.universe?.watching_count ?? 0) > 0 && scannerOk ? 'bg-emerald-400' : 'bg-amber-400',
          },
          {
            label: 'Scanner',
            ok: scannerOk,
          ...
          <div key={item.label} className="bg-[#141b2d] border border-[#1e2d45] rounded-lg px-3 py-2 flex items-center gap-2">
            ...
            <div className="min-w-0">
              <div className="text-[10px] text-slate-500">{item.label}</div>
              <div className={`text-xs font-medium truncate ${item.color}`}>{item.val}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {scannerHealth && (scannerHealth.scan_count ?? 0) > 0 && (
        <div className="bg-[#141b2d] border border-[#1e2d45] rounded-lg px-4 py-2.5 flex items-center gap-3">
          <Heart size={14} className={`flex-shrink-0 ${scannerOk ? 'text-emerald-400' : 'text-amber-400'}`} />
          <div className="text-xs text-slate-400 flex flex-wrap gap-x-4 gap-y-0.5">
            <span>Scans completed: <span className="text-white font-medium">{scannerHealth.scan_count}</span></span>
            {scannerHealth.last_scan_seconds_ago != null && (
              <span>Last scan: <span className="text-white font-medium">{Math.round(scannerHealth.last_scan_seconds_ago)}s ago</span></span>
            )}
            {(scannerHealth.consecutive_failures ?? 0) > 0 && (
              <span className="text-amber-400">Consecutive failures: <span className="font-medium">{scannerHealth.consecutive_failures}</span></span>
            )}
            {scannerHealth.uptime_seconds != null && scannerHealth.uptime_seconds > 0 && (
              <span>Scanner uptime: <span className="text-white font-medium">{Math.floor(scannerHealth.uptime_seconds / 60)}m</span></span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-[#141b2d] border border-[#1e2d45] rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e2d45]">
          ...
          <div className={`flex items-center gap-1.5 text-xs ${isWsConnected ? 'text-emerald-400' : 'text-slate-500'}`}>
            {isWsConnected ? <Wifi size={11} /> : <WifiOff size={11} />}
            {isWsConnected ? 'Live' : 'Polling feed'}
          </div>
        </div>
