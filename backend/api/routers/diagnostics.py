...
async def _test_historical_data() -> Dict[str, Any]:
    ...
    try:
        ...
        if candles and len(candles) > 0:
            latest = candles[-1]
            detail = (f"Fetched {len(candles)} daily candles for {underlying}. "
...
async def _test_risk_manager() -> Dict[str, Any]:
    ...
    try:
        ...
        allowed, reason = rm.can_take_trade("NIFTY50")
        ms = (time.monotonic() - t0) * 1000
...
async def environment_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "environment": {key: bool(os.getenv(key)) for key in keys},
    }


@router.get("/upstox-auth")
async def upstox_auth_diagnostic() -> Dict[str, Any]:
    """Diagnostic endpoint to perform a real authenticated Upstox request against /v2/user/profile.
    Never exposes full token. Returns authentication status, source, fingerprint, and response code.
    """
    ...
    if not token:
        return {
            ...
            "http_status": None,
            "error_code": "NO_TOKEN",
            "message": "No Upstox access token found in database or environment.",
            "token_source": meta["source"],
            "token_fingerprint": meta["fingerprint"],
            "token_length": meta["length"],
    ...
    try:
        ...
        if http_status == 200 and body.get("status") == "success":
            ...
            message = "Authentication successful. Active broker session verified."
        else:
            if "errors" in body and isinstance(body["errors"], list) and len(body["errors"]) > 0:
                err_code = body["errors"][0].get("errorCode") or body["errors"][0].get("error_code")
                message = body["errors"][0].get("message") or "Authentication failed"
            elif http_status == 401:
                err_code = "UDAPI100050"
                message = "Invalid or expired access token"
            else:
                err_code = f"HTTP_{http_status}"
                message = body.get("message", "Upstox API returned an error")

        return {
            "authenticated": (http_status == 200),
        ...
        return {
            ...
            "message": f"Failed to connect to Upstox API: {e}",
            "token_source": meta["source"],
            "token_fingerprint": meta["fingerprint"],
            "token_length": meta["length"],
            "timestamp": datetime.now(timezone.utc).isoformat(),
