...
def test_validator_rejects_spot_price_substitution():
...
def test_validator_rejects_invalid_ohlc_and_duplicates():
    ...
    assert not valid2
    assert "Duplicate timestamp" in err2


def test_validator_contract_metadata():
    ...
    assert not ok_bad_strike
    assert "Strike mismatch" in err_strike
    ...
    assert not ok_bad_type
    assert "Option type mismatch" in err_type


def test_options_cache_deterministic_naming_and_persistence(tmp_path):
    """Cache must store and retrieve historical option candles using deterministic file names."""
    cache = OptionsDataCache(cache_dir=str(tmp_path))
    ...
    assert filename == "NIFTY50_20240627_24500_CE_5minute_20240625_20240627.json"
    ...
    assert loaded["contract"]["strike"] == 24500.0
    assert len(loaded["candles"]) == 2
    assert loaded["candles"][0]["close"] == 190.0


def test_single_contract_mock_execution_and_price_validation():
    ...
    assert c_key == "NSE_FO|NIFTY2462724500CE"
    assert exp_str == "2024-06-27"
    ...
    assert candle.close == 215.0
    assert candle.volume == 75000
    assert candle.oi == 125000
