...
This strategy trades an index option premium. It needs more context than a
plain candle series:
...
class OptionPremiumStrategy(Strategy):
    ...
    def _is_expiry_day(
        ...
        # Explicit evaluation date passed in context (e.g. current_bar_date in backtest, live date in live trading)
        eval_val = (
            context.get("current_bar_date")
            or context.get("evaluation_date")
            or context.get("current_date")
        )
        eval_date = _to_date(eval_val)

        # If not explicit in context, extract date from the current candle timestamp if available
        if not eval_date and candles and len(candles) > 0:
            ts = candles[-1].get("timestamp")
            eval_date = _to_date(ts)

        # If neither context nor candle provides a date, fallback to live trading date
        if not eval_date:
            eval_date = date.today()

        exp_d = _to_date(expiry_date)
        if exp_d and eval_date:
            return exp_d == eval_date
        return False
